        ...
        await bank.forEach(element => {
            if (check) {
                if (document.querySelector(`.ccy-${e.target.parentElement.parentElement.className}`).innerText === element.ccy) {
                    document.querySelector(`.${e.target.parentElement.parentElement.className}-${element.ccy}-INPUT2`).value = document.querySelector(`.${element.ccy}-INPUT1`).value / element.sale;
                }
            } else {
                if (document.querySelector(`.ccy-${e.target.parentElement.parentElement.className}`).innerText === element.ccy) {
                    document.querySelector(`.${e.target.parentElement.parentElement.className}-${element.ccy}-INPUT4`).value = document.querySelector(`.${e.target.parentElement.parentElement.className}-${element.ccy}-INPUT3`).value * element.buy;
                }
            }
        });
    })  
